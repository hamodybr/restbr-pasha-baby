...
requireText('js/admin-orders-customers.js', '@page{size:${pageSize};margin:${cfg.page_margin_mm}mm}', 'configurable laser-printer page sizing');
requireText('js/admin-orders-customers.js', 'window.PashaInvoiceSettings?.normalize', 'saved invoice settings integration');
requireText('js/admin-orders-customers.js', 'font-family:${invoiceFont}', 'configurable Arabic invoice font');
requireText('js/admin-orders-customers.js', 'min-height:${cfg.row_min_height_mm}mm', 'configurable compact invoice rows');
requireText('js/admin-orders-customers.js', 'border-bottom:${cfg.leader_width_pt}pt ${cfg.leader_style}', 'configurable invoice item leaders');
...
requireText('js/admin-invoice-settings.js', 'mergeIntoUiDesignSettings', 'persistent invoice settings merge');
requireText('admin.html', 'window.PashaInvoiceSettings?.uploadPendingAssets?.()', 'invoice asset save integration');
requireText('supabase/migrations/20260910193840_invoice_assets_bucket.sql', "'font/woff2'", 'invoice font storage mime types');
...
requireText('js/pasha-arabic-only.js', "js/admin-orders-customers.js?v=1.7", 'configurable invoice admin orders loader');
