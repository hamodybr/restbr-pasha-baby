...
if (!promo || promo.hidden) {
  fail('Promo banner did not detect the real percentage discount.');
}
if (document.getElementById('pbV3PromoTitle')?.textContent !== 'حفاضات أطفال') {
  fail('Promo banner title did not come from the discounted product.');
}
if (!document.getElementById('pbV3PromoImage')?.src.includes('diaper.webp')) {
  fail('Promo banner image did not come from the discounted product.');
}
if (document.getElementById('pbV3PromoDiscount')?.textContent !== '-17%') {
  fail('Promo banner percentage did not reflect real discount data.');
}
if (!document.getElementById('pbV3PromoMeta')?.textContent.includes('قبل 12,000') ||
    !document.getElementById('pbV3PromoMeta')?.textContent.includes('الآن 10,000')) {
  fail('Promo banner did not show real old/current prices.');
}
if (document.getElementById('pbV3OfferSection')?.hidden ||
  ...
  fail('Offer rail did not detect the real percentage discount.');
}
...
if (openedPromoProduct !== 'p1') {
  fail('Promo banner details action did not open the discounted product.');
}

window.RESTBR_DB.products[0].discountPercent = 0;
window.RESTBR_DB.products[0].discountAmount = 0;
window.RESTBR_DB.products[0].options[0].originalPrice = 10000;
window.RESTBR_DB.products[0].options[0].price = 10000;
...
window.RESTBR_DB.products[0].discountAmount = 2000;
window.RESTBR_DB.products[0].options[0].originalPrice = 12000;
window.RESTBR_DB.products[0].options[0].price = 10000;
window.dispatchEvent(new window.CustomEvent('restbr:prices-updated'));
await new Promise(resolve => setTimeout(resolve, 0));

if (document.getElementById('pbV3PromoBanner')?.hidden ||
    document.getElementById('pbV3PromoDiscount')?.textContent !== 'خصم') {
  fail('Promo banner did not detect a fixed-IQD discount.');
}
if (!document.getElementById('pbV3PromoMeta')?.textContent.includes('قبل 12,000') ||
    !document.getElementById('pbV3PromoMeta')?.textContent.includes('الآن 10,000')) {
  fail('Fixed-IQD promo did not preserve real old/current prices.');
}
