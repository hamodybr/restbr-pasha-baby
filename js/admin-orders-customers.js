  ...
  function printOrder(orderId) {
    ...
    popup.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(order.order_number)} — Pasha Baby</title><style>
      ...
      *{box-sizing:border-box}
      html,body{margin:0;padding:0;min-width:0;background:#fff;color:#000;font-family:"Modern Pro Bold","Modern Pro","DIN Next Arabic","Geeza Pro",Tahoma,Arial,sans-serif;font-size:12pt;font-weight:900;line-height:1.25;font-variant-numeric:tabular-nums;-webkit-print-color-adjust:exact;print-color-adjust:exact}
      body{padding:8mm 8mm 25mm}
      .label{position:relative;width:100%;max-width:198mm;min-height:273mm;margin:0 auto;padding:7mm 8mm 6mm;background:#fff;color:#000;border:3pt double #000;border-radius:4mm}
      .brand{text-align:center;break-inside:avoid}.print-logo-mark{position:relative;width:27mm;height:27mm;margin:0 auto 2mm;border:3pt double #000;border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;font-family:Georgia,"Times New Roman",serif;font-weight:900;line-height:.95}.print-logo-mark::before{content:"♛";display:block;font-size:14pt;line-height:1}.print-logo-name{font-size:10pt;letter-spacing:.2pt;white-space:nowrap}.print-logo-pb{font-size:15pt;font-style:italic}.brand-copy h1{margin:0;color:#000;font:900 25pt/.95 Georgia,"Times New Roman",serif;letter-spacing:1.2pt}.brand-ar{margin-top:1mm;font:900 8pt/1.1 Georgia,"Times New Roman",serif;letter-spacing:2pt}.ornament{display:flex;align-items:center;gap:3mm;margin:2.5mm 0 2mm}.ornament::before,.ornament::after{content:"";height:1pt;background:#000;flex:1}.ornament span{font-size:10pt;line-height:1}
      .orderline{display:flex;justify-content:space-between;align-items:center;gap:4mm;margin-bottom:1.4mm;font-size:8pt;font-weight:900;break-inside:avoid}.orderline strong{font:900 8pt/1.15 ui-monospace,SFMono-Regular,Consolas,monospace;direction:ltr;text-align:left}.orderline span{font-size:8pt;font-weight:900}.customer{border-top:1pt solid #000;border-bottom:1pt solid #000;padding:1.5mm 0;margin-bottom:2mm;font-size:10.5pt;font-weight:900;line-height:1.35;break-inside:avoid}.customer-main{display:flex;justify-content:space-between;gap:4mm}.customer b{font-size:11.5pt;font-weight:900}.phone{direction:ltr;display:inline-block;font-weight:900}.address{margin-top:.7mm;font-size:9.5pt;font-weight:900}
      .details-title{text-align:center;margin:0 0 1mm;font-size:17pt;font-weight:900}.items{padding:0 1mm}.item{display:flex;align-items:baseline;gap:2mm;min-height:7mm;padding:1mm 0;font-size:12pt;font-weight:900;line-height:1.25;break-inside:avoid}.item-copy{display:flex;align-items:baseline;gap:1.2mm;min-width:0}.item-copy::before{content:"◆";font-size:7pt;flex:none}.item b{font-weight:900}.item-leader{min-width:12mm;flex:1;border-bottom:1.5pt dotted #000;transform:translateY(-1.2mm)}.item strong{min-width:27mm;white-space:nowrap;font-weight:900;direction:ltr;text-align:left}.option{display:inline;font-size:9.5pt;color:#000;font-weight:900}.option::before{content:" — "}.delivery-item,.delivery-item .option{color:#000}
      .notes{margin-top:1.5mm;padding:1.5mm 0;border-top:1pt solid #000;font-size:9.5pt;font-weight:900;line-height:1.3;break-inside:avoid}.totals{margin-top:2mm;border:2pt solid #000;border-radius:2mm;padding:2mm 4mm;display:grid;gap:1mm;break-inside:avoid}.row{display:flex;justify-content:space-between;gap:5mm;font-size:11pt;font-weight:900}.row b{white-space:nowrap;direction:ltr}.row.grand{font-size:16pt;font-weight:900}.footer{text-align:center;margin-top:2.5mm;font-size:15pt;font-weight:900;break-inside:avoid}.footer::before{content:"◆";display:block;font-size:9pt;margin-bottom:1mm}.screen-actions{display:flex;gap:8px;padding:12px;position:fixed;left:0;right:0;bottom:0;background:#eee;z-index:5}.screen-actions button{flex:1;padding:12px;border:0;border-radius:8px;background:#000;color:#fff;font-size:16px;font-weight:900}
      @media print{html,body{width:auto!important;min-width:0!important;background:#fff!important;color:#000!important}body{padding:0!important}.label{width:100%!important;max-width:none!important;min-height:285mm!important;margin:0!important;overflow:visible!important}.screen-actions{display:none!important}}
    </style></head><body><div class="label">
      <div class="brand" aria-label="Pasha Baby"><div class="print-logo-mark" aria-hidden="true"><span class="print-logo-name">PASHA BABY</span><span class="print-logo-pb">PB</span></div><div class="brand-copy"><h1>PASHA BABY</h1><div class="brand-ar">PREMIUM BABY BOUTIQUE</div></div></div>
      ...
      <div class="customer"><div class="customer-main"><b>${invoiceEsc(order.customer_name)}</b><span><span class="phone">${invoiceEsc(order.customer_phone)}</span> · ${order.order_type === 'delivery' ? 'توصيل' : 'استلام'}</span></div>${order.address ? `<div class="address">العنوان: ${invoiceEsc(order.address)}</div>` : ''}</div>
      <div class="details-title">تفاصيل الطلب</div>
      <div class="items">${items.map(item => { const option = itemOptionText(item); return `<div class="item"><span class="item-copy"><b>${Number(item.quantity || 0)}× ${invoiceEsc(item.product_name)}</b>${option ? `<span class="option">${invoiceEsc(option)}</span>` : ''}</span><span class="item-leader" aria-hidden="true"></span><strong>${money(item.line_total)}</strong></div>`; }).join('')}${fee > 0 ? `<div class="item delivery-item"><span class="item-copy"><b>1× التوصيل</b><span class="option">خدمة التوصيل</span></span><span class="item-leader" aria-hidden="true"></span><strong>${money(fee)}</strong></div>` : ''}</div>
      ...
      <div class="footer">شكراً لاختياركم</div>
