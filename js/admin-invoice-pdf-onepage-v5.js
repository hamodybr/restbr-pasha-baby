(() => {
  const RENDER_MODE = 'native-canvas-one-page-v5';
  if (window.PashaInvoiceOnePagePdf?.renderMode === RENDER_MODE) return;

  const PX_PER_MM = 96 / 25.4;
  const PT_TO_PX = 96 / 72;
  const CUSTOM_FAMILY_NAME = 'Pasha Invoice Raster V5';
  ...
  async function prepareFont(fontBuffer, cfg) {
    ...
    let face;
    try {
      face = new FontFace(CUSTOM_FAMILY_NAME, fontBuffer.slice(0), {
        style: 'normal',
        weight,
        display: 'block'
      });
      const loaded = await timeout(face.load(), 10000, 'انتهت مهلة تحميل الخط المرفوع.');
      document.fonts.add(loaded);
      await timeout(document.fonts.ready, 5000, 'انتهت مهلة تثبيت الخط المرفوع.');
    } catch (error) {
    ...
    try {
      await document.fonts.load(probe, 'باشا بيبي تفاصيل الطلب محمد Mustafa 123');
      if (!document.fonts.check(probe, 'باشا بيبي تفاصيل الطلب محمد Mustafa 123')) {
        throw new Error('فشل التحقق من الخط بعد تحميله.');
      }
    } catch (error) {
      throw new Error(`الخط المرفوع لم يثبت فعلياً؛ تم إيقاف PDF حتى لا يُستخدم خط بديل. ${error?.message || ''}`.trim());
    }
  ...
  async function loadImage(dataUrl) {
    ...
    await timeout(new Promise((resolve, reject) => {
      ...
      image.src = dataUrl;
    }), 8000, 'انتهت مهلة تجهيز شعار الفاتورة.');
    return image;
  }
  ...
  function renderInvoiceCanvas(options, font, logoImage) {
    ...
    const renderScale = Math.max(0.65, Math.min(desiredScale, pixelScale, dimensionScale));
    ...
    return { canvas, cssWidth, cssHeight, renderScale };
  }

  function canvasToSinglePagePdf(jsPDF, rendered, cfg) {
    ...
    doc.addImage(png, 'PNG', x, y, drawWidth, drawHeight, undefined, 'FAST');
    return {
  ...
  async function create(options) {
    ...
    const font = await prepareFont(fontBuffer, cfg);
    const logo = cfg.show_logo && cfg.logo_mode === 'image' && logoDataUrl ? await loadImage(logoDataUrl) : null;
  ...
  window.PashaInvoiceOnePagePdf = Object.freeze({
    renderMode: RENDER_MODE,
    create: options => timeout(create(options), 25000, 'تعذر إنهاء PDF خلال 25 ثانية على هذا الجهاز. حاول مرة أخرى.')
  });
})();
